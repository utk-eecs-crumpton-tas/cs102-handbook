...
- _Mac Users_ - You can skip to the [Installing `g++`](/cs102-writeups/docs/general/installing-g++?os=macos#installing-xcode-command-line-tools-g) section
- _Windows Users_ - Read on

:::info Windows Users...

This guide will cover installing `WSL`, which is a Linux virtual machine specifically for Windows. It can be a bit annoying to set up, but it's the only way you can get a Unix experience on Windows without SSH, which is important **if you're a CS major**. Otherwise, I would skip to the [Installing `g++` without WSL](/cs102-writeups/docs/general/installing-g++?os=windows#installing-g-without-wsl) section instead. `WSL` is not necessary for this course or to install `g++`.
...
If you're not familiar with VS Code or extensions, then read the [Pre-setup](/docs/general/vscode-setup).
...
If you want a reference for how to use `gdb`, you can check out <LinkWithIcon to="/docs/debugging/gdb" text="this guide we've written" />.
...
First let's check to see if it's already installed. Open up a terminal instance of Ubuntu and run the command `gdb`. If it works, skip to <LinkWithIcon to="/docs/debugging/gdb" text="this gdb guide" /> to get started.
...
First let's check to see if it's already installed. Open up a terminal instance (powershell, cmd, terminal, whatever) and run the command `gdb`. If it works, skip to <LinkWithIcon to="/docs/debugging/gdb" text="this gdb guide" /> to get started.
...
First let's check to see if it's already installed. Open up a terminal instance and run the command `lldb`. If it works, skip to <LinkWithIcon to="/docs/debugging/gdb" text="this gdb guide" /> to get started.

If it doesn't work, follow the instructions <LinkWithIcon to="/cs102-writeups/docs/general/installing-g++?os=macos#installing-xcode-command-line-tools-g" text="here" /> to install the Xcode command line tools.
