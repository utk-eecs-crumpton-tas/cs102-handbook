...
I want you to think of `fstream` as the same thing as `iostream` but for files. The two work almost identically, just slight variations. Similar to `cout` and `cin`, `fstream` has `ifstream` and `ofstream`. `ifstream` is for reading from a file, and `ofstream` is for writing to a file. We'll be using `ifstream` in this lab.
...
- **Year** - The year the ticket was issued. This is the fourth value in the input file, and is in the format `yyyy`.
- **Citation** - The citation number (not a number). This is the first value in the input file.
- **Fine** - The fine you calculated for the ticket.
...
Navigate to your `lab2` directory and copy/paste this _entire_ command into your Ubuntu terminal. This will download the files and save them in your current directory.

```bash
sudo apt install curl -y && \
curl https://pastebin.com/raw/whKWBRR6 --output ticket.txt && \
curl https://pastebin.com/raw/QBQAQr0i --output test.txt
```
...
Navigate to your `lab2` directory and run these two commands in your PowerShell terminal. This will download the files and save them in your current directory.

```pwsh
Invoke-WebRequest https://pastebin.com/raw/whKWBRR6 -OutFile ticket.txt
Invoke-WebRequest https://pastebin.com/raw/QBQAQr0i -OutFile test.txt
```
...
Navigate to your `lab2` directory and copy/paste this _entire_ command into your terminal. This will download the files and save them in your current directory.
...
Now that you've downloaded the testing files, you can use them to test your program almost automatically.
