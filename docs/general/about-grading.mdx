...
Ethan has talked a bit about automatic-code formatters for VS Code in <LinkWithIcon to="/docs/general/pre-setup#extra" text="this document" /> as well, which is basically the `gg=G` command but for VS Code.
