...
`gdb` is program that allows you to debug your own programs. It's a dynamic debugger, which means it attaches to your program as it runs, and you can interact with your program like a surgeon operating on a patient.

In this guide, I'll cover

- Installing `gdb` for both MacOS & Windows
- `gdb` commands and how to use them

## Installing `gdb`

If you can't get `gdb` to work, or you don't want to install it on your computer, you have a couple options.

1. Use the lab machines. They have `gdb` installed already.
2. Use the <LinkWithIcon text="online gdb debugger" to="https://www.onlinegdb.com/online_c++_compiler" />. It's a little bit slower, but it works.
...
First let's check to see if it's already installed. Open up a terminal instance (powershell, cmd, terminal, whatever) and run the command `gdb`. If it works, skip to <LinkWithIcon to="#gdb-flowchart" text="the flowchart" /> to get started.
...
First let's check to see if it's already installed. Open up a terminal instance of Ubuntu and run the command `gdb`. If it works, skip to <LinkWithIcon to="#gdb-flowchart" text="the flowchart" /> to get started.
...
First let's check to see if it's already installed. Open up a terminal instance and run the command `lldb`. If it works, skip to <LinkWithIcon to="#gdb-flowchart" text="the flowchart" /> to get started.

If it doesn't work, follow the instructions <LinkWithIcon to="/cs102-writeups/docs/general/info?os=macos#installing-xcode-command-line-tools-g" text="here" /> to install the Xcode command line tools.
...
Lastly, of the `Debug Options` commands can be repeated by pressing `↩` (enter) without typing anything.
...
```mermaid
graph TD
Start[Start Debugging with gdb] -->|gdb :binary_name:| LoadProgram{LoadProgram} --> PreRun[Pre-Run]
PreRun --> |break/b :line number or function name:| SetBreakpoint[Set Breakpoint]
PreRun --> |"run/r :argument(s):"| StartExecution[Start Execution]
...
During this phase, you'll set any <LinkWithIcon to="#setting-breakpoints" text="breakpoints" /> you want to set, and then start the execution of your program.
...
To set a breakpoint, you'll use the `break` command. You can also use the `b` command, which is a shortcut for `break`.
...
Reading it's output, you can see that the crash happened on line 50 of the file `../sysdeps/unix/sysv/linux/raise.c`. And you can see that the function that called `raise` was `abort`, which was called by `libc_message` — hence the back**trace**.
